...
def play_game(draw_thresh,search_iters,model=None,game_num=0,half_precision=False):
    ...
    while not (game_board.is_checkmate() or game_board.is_stalemate() or game_board.is_seventyfive_moves() or game_board.is_fifty_moves()):
        ...
        MCtree              = rl.Tree(game_board,model,base_node=MCtree.root.children[next_move_i],draw_thresh=draw_thresh,lookups=MCtree.lookups,device=DEV)
    #Check game outcome 
...
def train(model:networks.FullNet,n_samples,gen,bs=8,epochs=5):
    ...
    for epoch_i in range(epochs):
        ...
        for batch_i,batch in enumerate(dataloader):
            ...
            #Calc model loss 
            loss                        = torch.nn.functional.mse_loss(v_pred.view(-1),outcome) + torch.nn.functional.cross_entropy(pi_pred,pi)
