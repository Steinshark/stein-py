...
def run_training(search_depth,move_limit,game_id,lookup_table=None,lock=None):
...
if __name__ == "__main__":
	...
	model 			= ChessNet(optimizer_kwargs={"lr":.001,"weight_decay":.0001})
	...
	if sys.argv[1] == "train":
		load_model(model,2,verbose=True)
		train(model,8096*2,1,32,epochs=5)
		save_model(model,0)
	
	elif sys.argv[1] == "sim":
		train_iters 	= 20 
		load_model(model,0,verbose=True)
